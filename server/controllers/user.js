...
export const submitContactForm = async (req, res) => {
  ...
  // Function to handle contact form submission
    ...
    try {
      // Call the contact API function to save the form data
      response = await user.contact({ name, email, message });

      // Optionally, send a confirmation email to the user
      sendMail({
        to: email,
        subject: 'Thank you for contacting LawGPT!',
        html: `<p>Dear ${name},</p><p>Thank you for reaching out to us. We have received your message and will get back to you shortly.</p>`,
      });
      ...
      if (err?.exists) {
        res.status(400).json({
          status: 400,
          message: 'Duplicate entry. You have already contacted us.',
        });
      } else {
        ...
        fs.readFile(
          ...
          (err, html) => {
            if (!err) {
              ...
              sendMail({
                to: req.body.email,
                subject: `Change password for LawGPT Account`,
                html,
              });
            } else {
              console.log(err);
